<x-layouts.app title="Profile" pageTitleName="Profile" sidebarShow=true>
    ...
    @endpush
    <div class="w-100">
        <a href="{{route('data.siswa.index')}}" class="btn btn-success"><i class="bi bi-arrow-left mx-2"></i> Kembali</a>
    </div>
    <form id="profileForm" action="{{route('data.siswa.update')}}" method="POST" enctype="multipart/form-data">
        @csrf
        {{-- @method('PUT') --}}
            ...
            <div class=" mx-auto">
                <div class="mx-auto overflow-hidden rounded-circle ratio ratio-1x1 p-2 d-flex justify-content-center mt-5 profile-pic">
                    @if ($siswa->user->foto_profile)
                        <img id="currentProfileImage" class="current-image" alt="Foto Profile" src="{{ asset('storage/profile-images/' . $siswa->user->foto_profile) }}">
                    @else
                    ...
                    {{-- <img src="{{ asset('storage/profile-images/' . $siswa->user->foto_profile) }}" alt="Foto Profil" width="150"> --}}
            ...
            <div class="info">
                <div class="p-2">
                    <div class="form border shadow-sm rounded-2 p-2">
                        ...
                        <input type="text" hidden id="idSiswa" name="idSiswa" value="{{$id}}">
                        ...
                        <div class="row">
                            <div class="form-group mb-3 col">
                                ...
                                <input type="text" name="nama_lengkap" class="form-control" value="{{ $siswa->name }}" readonly>
                            </div>

                            <div class="form-group mb-3 col">
                                <label for="username" class="form-label">Username</label>
                                <input type="text" name="username" class="form-control" value="{{ $siswa->user->username ?? '' }}" readonly>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group mb-3 col">
                                <label for="nip" class="form-label">NISN</label>
                                <input type="text" name="nip" class="form-control" value="{{ $siswa->nisn ?? ''}}" readonly>
                            </div>

                            <div class="form-group mb-3 col">
                                <label for="email" class="form-label">Email</label>
                                <input type="email" name="email" class="form-control" value="{{ $siswa->user->email ?? ''}}" readonly>
                            </div>
                        </div>
